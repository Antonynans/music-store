...
const HeroBanner: React.FC = () => {
  ...
  return (
    <main
      className="heroBg min-h-[calc(100vh-77px)] bg-no-repeat bg-contain bg-bottom z-10 mb-40 bg-black
    ...
    >
      ...
      <div className="xl:w-2/3 w-11/12 container mx-auto flex lg:flex-row flex-col justify-between items-center">
        <div className="text-white min-h-[calc(100vh-130px)] lg:min-h-full flex flex-col justify-center gap-8 items-center text-center lg:text-left lg:items-start">
          <p className="font-bold text-[#6d6d6d] text-sm tracking-[0.65rem]">
            New product
          </p>
          <p className="flex flex-col text-5xl leading-14 max-w-88">
            XX99 Mark II Headphones
          </p>
          <p className="text-[#6d6d6d] leading-6 max-w-88">
            Experience natural, lifelike audio and exceptional build quality
            made for the passionate music enthusiast.
          </p>
          <button
            className="px-8 py-4 bg-orange-500 uppercase rounded-sm hoverBtn"
            onClick={() => router.push("/product/xx99-mark-two-headphones")}
          >
            See Product
          </button>
        </div>
        <div className="hidden lg:block max-w-120 mt-8">
          <Image
            src={home_desktop_images.image_headset}
            alt="headset"
            placeholder="blur"
          />
        </div>
